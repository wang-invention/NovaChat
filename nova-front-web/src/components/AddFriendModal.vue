        ...
        <div v-if="results.length > 0" class="search-results">
          <div v-for="user in results" :key="user.id || user.userId" class="search-result-item">
            ...
            <span
              ...
              :class="{ added: user.added }"
              @click="!user.added && addFriend(user)"
            >{{ user.added ? '已添加' : '添加' }}</span>
          </div>
        </div>
...
async function doSearch() {
  ...
  try {
    ...
    results.value = (res.data || []).map(u => ({ ...u, added: false }))
    searched.value = true
  } catch {
    results.value = []
    searched.value = true
...
async function addFriend(user) {
  try {
    await sendFriendRequest(user.id || user.userId, '你好，我想加你为好友')
    const idx = results.value.findIndex(u => (u.id || u.userId) === (user.id || user.userId))
    if (idx >= 0) results.value[idx].added = true
  } catch {}
}
</script>